...
def request_ping(data_type,data_code,data_checksum,data_ID,data_Sequence,payload_body):
    #  把字节打包成二进制数据
    imcp_packet = struct.pack('>BBHHH16s',data_type,data_code,data_checksum,data_ID,data_Sequence,payload_body)
    icmp_chesksum = chesksum(imcp_packet)  #获取校验和
    #  把校验和传入，再次打包
    imcp_packet = struct.pack('>BBHHH16s',data_type,data_code,icmp_chesksum,data_ID,data_Sequence,payload_body)
    return imcp_packet
...
def reply_ping(send_request_ping_time,rawsocket,data_Sequence,timeout = 2):
    while True:
        ...
        decode_payload = key0.decrypt(payload, padding=True)
        # print(payload)
        print("接收方返回:", decode_payload)
...
def ping(fileName):
    ...
    pingTime = math.ceil(fileLen / batch_size)  # ping 的次数
    ...
    code = 0 # must be zero
    checksum = 0 # "...with value 0 substituted for this fie19ld..."
    ID = 0 #Identifier
    sequence = 1 #Sequence number
    # 将主机名转ipv4地址格式，返回以ipv4地址格式的字符串，如果主机名称是ipv4地址，则它将保持不变
    dst_addr = socket.gethostbyname(dst_addr1)
    print("正在 Ping [{0}] 具有 16 字节的数据:".format(dst_addr))
    for i in range(0,4):
        start = i * batch_size
        ...
        print("发送:", payload_body)
        # 请求ping数据包的二进制转换
        icmp_packet = request_ping(type, code, checksum, ID, sequence + i, encode_payload)
        #连接套接字,并将数据发送到套接字
        send_request_ping_time,rawsocket,addr = raw_socket(dst_addr,icmp_packet)
        #数据包传输时间
        times = reply_ping(send_request_ping_time,rawsocket,sequence + i)
        if times > 0:
